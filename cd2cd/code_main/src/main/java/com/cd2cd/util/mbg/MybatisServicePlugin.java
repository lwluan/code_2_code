...
public class MybatisServicePlugin extends PluginAdapter {
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
		...
		pojoCriteriaType = new FullyQualifiedJavaType(introspectedTable.getExampleType());//pojoUrl + "." + tableName + "Criteria");
		listType = new FullyQualifiedJavaType("java.util.List");

		String genPackage = servicePack + ".gen" ;
		FullyQualifiedJavaType genInterfaceType = new FullyQualifiedJavaType(genPackage + ".Super" + tableName + "Service");
		
		genPackage = serviceImplPack + ".gen";
		FullyQualifiedJavaType genTopLevelClass = new FullyQualifiedJavaType(genPackage + ".Super" + tableName + "ServiceImpl");
		
		
		FullyQualifiedJavaType curInterfaceType = new FullyQualifiedJavaType(servicePack + "." + tableName + "Service");
		Interface curInterface = new Interface(curInterfaceType);
		JavaFormatter javaFormatter = new DefaultJavaFormatter();
		
		
		// 新建自定义继承接口类
		// = = = 判断是否存在
		String extFilePath = (project + genTopLevelClass.getFullyQualifiedName()) .replaceAll("\\.", "/") + ".java";
		if( ! new File(extFilePath).exists()) {
			curInterface.setVisibility(JavaVisibility.PUBLIC);
			curInterface.addImportedType(genInterfaceType);
			curInterface.addSuperInterface(genInterfaceType);
			GeneratedJavaFile file = new GeneratedJavaFile(curInterface, project, javaFormatter);
			files.add(file);
		}
		
		// 新建自定义service 继承
		extFilePath = (project + genTopLevelClass.getFullyQualifiedName()) .replaceAll("\\.", "/") + ".java";
		if( ! new File(extFilePath).exists()) {
			FullyQualifiedJavaType curClassType = new FullyQualifiedJavaType(serviceImplPack + "." + tableName + "ServiceImpl");
			TopLevelClass curTopLevelClass = new TopLevelClass(curClassType);
			curTopLevelClass.setVisibility(JavaVisibility.PUBLIC);
			curTopLevelClass.setSuperClass(genTopLevelClass);
			curTopLevelClass.addSuperInterface(curInterfaceType);
			curTopLevelClass.addImportedType(curInterfaceType);
			curTopLevelClass.addImportedType(genTopLevelClass);
			if (enableAnnotation) {
				curTopLevelClass.addAnnotation("@Service");
				curTopLevelClass.addImportedType(service);
			}
			GeneratedJavaFile file = new GeneratedJavaFile(curTopLevelClass, project, javaFormatter);
			files.add(file);
		}
		
		Interface interface1 = new Interface(genInterfaceType);
		...
		addServiceImpl(genInterfaceType, topLevelClass, introspectedTable, tableName, files);
		return files;
	}
	...
	protected void addService(Interface interface1, IntrospectedTable introspectedTable, String tableName, List<GeneratedJavaFile> files) {
		...
		JavaFormatter javaFormatter = new DefaultJavaFormatter();
		GeneratedJavaFile file = new GeneratedJavaFile(interface1, project, javaFormatter);
		files.add(file);
	}
	 ...
	 * @param files
	 */
	protected void addServiceImpl(FullyQualifiedJavaType interfaceType, TopLevelClass topLevelClass,IntrospectedTable introspectedTable, String tableName, List<GeneratedJavaFile> files) {
		...
		JavaFormatter javaFormatter = new DefaultJavaFormatter();
		GeneratedJavaFile file = new GeneratedJavaFile(topLevelClass, project, javaFormatter);
		files.add(file);
	}
	...
	protected void addComment(JavaElement field, String comment) {
		...
		sb.append(" * ");
		comment = comment.replaceAll(Constants.lineSeparator, "<br>"+Constants.lineSeparator+"\t * ");
		sb.append(comment);
